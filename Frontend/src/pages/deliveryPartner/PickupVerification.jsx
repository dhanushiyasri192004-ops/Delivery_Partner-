...
import { verifyPickup } from '../../redux/slices/deliverySlice';
...
const PickupVerification = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Mock GPS coordinates for telemetry
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        formData.append('latitude', pos.coords.latitude);
        formData.append('longitude', pos.coords.longitude);
        dispatch(verifyPickup({ orderId, formData })).then((res) => {
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-bold text-slate-800 text-lg">Verify Package Pickup</h3>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          />
          <p className="text-[10px] text-slate-400 mt-1 text-center">Ask the merchant for the unique pickup code.</p>
        </div>

        {/* Photo Upload Capture (Matches wireframe) */}
        <div>
          <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
            Capture Photo Proof
          </label>
          ...
          {preview ? (
            <div className="relative rounded-xl overflow-hidden border border-slate-200">
              <img src={preview} alt="Pickup Proof" className="w-full h-48 object-cover" />
              <button 
                type="button" 
                onClick={() => { setPhoto(null); setPreview(null); }}
                className="absolute top-2 right-2 bg-red-600 text-white p-1 rounded-full text-xs font-bold px-2.5"
              >
                Clear
              </button>
            </div>
          ) : (
            <label className="w-full h-40 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-slate-100/80 transition-all text-slate-400">
              <Camera className="h-8 w-8 text-slate-300" />
              <span className="text-xs font-semibold text-slate-500">Tap to capture or upload package image</span>
